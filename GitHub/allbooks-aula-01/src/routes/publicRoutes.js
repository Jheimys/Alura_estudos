...
server.post('/cadastrar', async (req, res) => {
    ...
    if (emailExiste(email)) {
        const status = 401;
        const message = 'E-mail já utilizado!';
...
server.post('/login', (req, res) => {
    ...
    if (!usuarioExiste(email, senha)) {
        const status = 401
        const message = 'O e-mail ou senha está incorreto!'
